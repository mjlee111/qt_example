...
mainwindow::mainwindow(QWidget* parent)
  : QMainWindow(parent), ui(new Ui::mainwindow), startPoint(-1, -1), endPoint(-1, -1)
{
  ui->setupUi(this);
...
void mainwindow::drawGrid(int rows, int cols)
{
  ...
  for (int r = 0; r < rows; ++r)
  {
    for (int c = 0; c < cols; ++c)
    {
      ...
      cell->setStyleSheet("background-color: white; border: 0.5px solid black;");
      ...
      connect(cell, &QPushButton::clicked, [this, r, c, cell]() {
        if (!isSetStart && !isSetEnd)
        {
          ...
          map[r][c] = 1;
          cell->setStyleSheet("background-color: black; border: 0.5px solid black;");
        }
...
void mainwindow::replacePoint(int pointType, int newRow, int newCol, const QString& color)
{
  for (int r = 0; r < map.size(); ++r)
  {
    for (int c = 0; c < map[r].size(); ++c)
    {
      ...
      {
        map[r][c] = 0;
        QPushButton* cell = qobject_cast<QPushButton*>(ui->astar_layout->itemAtPosition(r, c)->widget());
        if (cell)
        {
          cell->setStyleSheet("background-color: white; border: 0.5px solid black;");
        }
  ...
  QPushButton* cell = qobject_cast<QPushButton*>(ui->astar_layout->itemAtPosition(newRow, newCol)->widget());
  if (cell)
  {
    cell->setStyleSheet(QString("background-color: %1; border: 0.5px solid black;").arg(color));
  }
}
...
void mainwindow::findPath()
{
  ...
  auto path = astar.findPath(grid, { startPoint.x(), startPoint.y() }, { endPoint.x(), endPoint.y() });

  if (path.empty())
  {
    QMessageBox::information(this, "No Path", "No path found between start and end points.");
    return;
  }

  for (const auto& p : path)
  {
    QPushButton* cell = qobject_cast<QPushButton*>(ui->astar_layout->itemAtPosition(p.first, p.second)->widget());
...
void mainwindow::on_astar_start_clicked()
{
  findPath();
}

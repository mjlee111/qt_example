...
Astar::reconstructPath(const std::vector<std::vector<Point>> &cameFrom,
                       const Point &current) {
...
std::vector<Astar::Point> Astar::findPath(const Grid &grid, const Point &start,
                                          const Point &end) {
  ...
  while (!openList.empty()) {
    ...
    for (const auto &neighbor : neighbors) {
      if (isValid(grid, neighbor) &&
          !closedList[neighbor.first][neighbor.second]) {
        double tentativeG = current.g + 1.0;
        double h = heuristic(neighbor, end);
        Node neighborNode(neighbor, tentativeG, h, pos);

        if (cameFrom[neighbor.first][neighbor.second].first == -1 ||
            tentativeG < neighborNode.g) {
